...
export default async function DashboardPage() {
  ...
  const [
    ...
    prisma.ticket.count({ where: { tenantId, assigneeId: session!.user.id, status: { in: openStatuses } } }),
    ...
    prisma.ticket.count({ where: { tenantId, priority: "CRITICAL", status: { in: openStatuses } } }),
    prisma.ticket.findMany({
      where: { tenantId, status: { in: openStatuses }, dueAt: { lt: now } },
      select: { id: true, number: true, subject: true },
      orderBy: { dueAt: "asc" },
      take: 4,
    }),
    prisma.ticket.findMany({
      where: { tenantId, status: { in: openStatuses }, dueAt: { gte: now, lte: endOfToday } },
      select: { id: true, number: true, subject: true },
      orderBy: { dueAt: "asc" },
      take: 4,
    }),
