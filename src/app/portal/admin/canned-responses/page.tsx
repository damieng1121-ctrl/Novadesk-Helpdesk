      ...
      <SlideOver open={open} onClose={() => setOpen(false)} title="Add a canned response" description="Saved reply text agents can insert with one click while replying to a ticket.">
        <form onSubmit={create} className="space-y-4">
          <div>
            ...
            <input required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Password reset instructions" className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">Shortcut</label>
            <input required value={shortcut} onChange={(e) => setShortcut(e.target.value.toLowerCase())} placeholder="password-reset" className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">Response text</label>
            <textarea required value={content} onChange={(e) => setContent(e.target.value)} rows={6} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </div>
